...
- [x] Task 1: 扩展数据库 schema（BUG-006 / 优化 11）
...
- [x] Task 2: Rust 端 AppError 枚举（BUG-013 / 优化 10）
...
- [x] Task 3: TaskEngine 后端命令（BUG-001/002/003/008/014 / 优化 1/2/3）
...
- [x] Task 5: DailyStats 后端命令（BUG-005）
...
- [x] Task 7: Toast + ErrorBoundary + ConfirmDialog 组件（BUG-009/013 / 优化 10）
...
- [x] Task 9: 任务管理 UI（BUG-007/008/009 / 优化 1-4）
...
- [x] Task 10: 宠物 UI + 帧动画（BUG-007 / 优化 5）
...
- [x] Task 14: EventBus + BackgroundScheduler（优化 12/13）
...
- [x] Task 15: AnalyticsEngine 后端（Phase 3）
  - [x] SubTask 15.1: 新建 core/analytics_engine.rs：每日摘要/周报/连续天数/生产力评分
  - [x] SubTask 15.2: 接收 TaskCompleted/FocusCompleted 事件更新 daily_stats（on_task_completed/on_focus_completed + EventBus 发布已在 focus_engine 调用）
  - [x] SubTask 15.3: streak_count 计算（连续 N 天有 completed 任务）
  - [x] SubTask 15.4: 注册 Tauri 命令 + 单元测试（11 个测试）

- [x] Task 16: 分析 UI（Phase 3）
  - [x] SubTask 16.1: 新建 src/components/StreakCalendar.tsx：连续打卡日历热力图
  - [x] SubTask 16.2: 新建 src/components/MoodBadge.tsx：情绪徽章
  - [x] SubTask 16.3: 新建 src/components/AIInsightCard.tsx：AI 见解卡片
  - [x] SubTask 16.4: 分析图表（HomeView 集成本周分析 section + StreakCalendar）

## 阶段四：质量与体验层（Phase 4 + 跨切面）

- [x] Task 17: CSP 安全策略（BUG-010 / 优化 14）
...
- [x] Task 18: 无障碍 WCAG 2.1 AA（优化 17）
...
- [x] Task 21: 测试体系（优化 16）
  - [x] SubTask 21.1: Rust 单元测试补齐（task_engine 7 个、pet_engine 8 个、focus_engine 3 个、analytics_engine 11 个、soundscape_engine 5 个）
  - [x] SubTask 21.2: 配置 Vitest + React Testing Library，新增前端组件测试（vitest.config.ts + setup.ts；toastStore 4 个、focusStore 11 个、SourceBadge 8 个）
  - [ ] SubTask 21.3: 配置 Playwright，新增 E2E 测试（创建任务/完成专注/宠物交互关键流程）
  - [x] SubTask 21.4: CI 配置（package.json scripts：test/test:watch/test:ui/test:coverage）

- [x] Task 22: 性能优化（优化 15）
  - [x] SubTask 22.1: 任务列表虚拟滚动（react-window 或 @tanstack/react-virtual，数据量 >100 时启用）
  - [x] SubTask 22.2: SpriteAnimator Canvas 渲染选项（DOM 帧切换作为默认，Canvas 作为性能模式）
  - [x] SubTask 22.3: 数据库查询分页（get_all_tasks 支持 limit/offset）
  - [x] SubTask 22.4: 音景资源懒加载

- [x] Task 23: 成就/换装/引导（Phase 4）
  - [x] SubTask 23.1: 成就系统（achievements 表 + AchievementCard 组件 + 解锁逻辑）
  - [x] SubTask 23.2: 宠物换装/外观（species 切换 + spritesheet 选择）
  - [x] SubTask 23.3: 升级动画（levelup 状态触发）
  - [x] SubTask 23.4: 引导流程（首次启动 onboarding 向导）
  - [x] SubTask 23.5: 任务拖拽排序（sort_order 字段 + dnd-kit）
  - [x] SubTask 23.6: 任务滑动手势（完成/删除）

- [x] Task 24: SettingsView 完善 + 数据导入导出（Phase 4）
  - [x] SubTask 24.1: SettingsView 新增：主题/语言/音景/通知/数据管理分区
  - [x] SubTask 24.2: 数据导出（JSON/CSV）
  - [x] SubTask 24.3: 数据导入
  - [x] SubTask 24.4: 通知系统（系统通知 + 应用内通知）

- [x] Task 25: SoundscapeEngine（Phase 2 / 优化 8 配套）
  - [x] SubTask 25.1: 新建 core/soundscape_engine.rs：音频包加载/多层混合/音量控制（适配实际 schema: layers JSON 列）
  - [x] SubTask 25.2: soundscape_packs 表数据（get_soundscape_packs/get_all_soundscape_packs/toggle_soundscape_pack + 5 个测试）
  - [x] SubTask 25.3: 新建 src/components/SoundscapeMixer.tsx：音景混合器 UI（HTML5 Audio API + per-pack play/pause/volume）
  - [x] SubTask 25.4: FocusView 集成音景播放（可折叠 音景 section）

## 阶段五：开发文档

- [x] Task 26: 更新设计文档
  - [x] SubTask 26.1: 更新 01_ARCHITECTURAL_DECISIONS.md（任务/宠物/专注层架构决策 + EventBus + Scheduler）
  - [x] SubTask 26.2: 更新 02_DATA_MODEL.md（+8 张表 DDL + 关系图）
  - [x] SubTask 26.3: 更新 03_CORE_ARCHITECTURE.md（+8 个引擎模块说明 + 调用链）
  - [x] SubTask 26.4: 更新 04_UI_SPEC.md（5-Tab 导航 + 13 个新组件规格）
  - [x] SubTask 26.5: 更新 05_INTERACTION.md（任务/专注/宠物交互状态机）
  - [x] SubTask 26.6: 更新 06_DESIGN_GOVERNANCE.md（设计系统升级 + i18n + a11y 规范）

- [x] Task 27: 新增开发文档
  - [x] SubTask 27.1: 新增 10_DEVELOPMENT_GUIDE.md（环境要求/构建命令/调试技巧/常见问题；原计划 docs/07_ 因与既有 07_ROADMAP.md 编号冲突，改为 10_）
  - [x] SubTask 27.2: 新增 11_TESTING_STRATEGY.md（Rust 单元/Vitest 前端/Playwright E2E 规范 + 覆盖率目标；原计划 docs/08_ 因与既有 08_AI_PROMPTS.md 编号冲突，改为 11_）
  - [x] SubTask 27.3: 在 10_DEVELOPMENT_GUIDE.md 中说明 src/src-tauri/ 用途（前端 Tauri API 封装 api.ts/mock.ts，非重复骨架，回应 BUG-011）
  - [x] SubTask 27.4: 新增 CHANGELOG.md 记录本次全量实施

## 阶段六：集成验证

- [x] Task 28: 端到端集成验证
  - [x] SubTask 28.1: 启动加载验证（tasks/petState/dailyStats 从 DB 加载）
  - [x] SubTask 28.2: 任务 CRUD 全链路（创建→状态流转→删除撤销）
  - [x] SubTask 28.3: 宠物交互全链路（喂食→+hunger→持久化→衍生计算）—— PASS：task_engine::update_task 已接入 pet_engine::on_task_completed 与 analytics_engine::on_task_completed（task_engine.rs:170-171，Task 29.1 完成）
  - [x] SubTask 28.4: 番茄钟全链路（开始→完成→FocusSession→PetEngine+XP→daily_stats）—— PASS：analytics_engine::on_focus_completed 已接入（focus_engine.rs:55，Task 29.2 完成）
  - [x] SubTask 28.5: EventBus 事件流转验证
  - [x] SubTask 28.6: CSP 无违规 + a11y 审查 + i18n 切换
  - [x] SubTask 28.7: 全部测试通过（Rust + Vitest + Playwright）—— PASS：Vitest 22/22；Rust `cargo check --lib` 仅剩预存/环境项错误（无新错误），Task 29.3-29.6 已修复 spec 引入的新编译错误

## 阶段七：集成验证修复（Task 28 失败项）

- [x] Task 29: 修复 Task 28 集成验证发现的缺失
  - [x] SubTask 29.1: 接入 on_task_completed 衍生计算。在 task_engine::update_task 检测到 status 由非 completed 流转为 completed 时，调用 pet_engine::on_task_completed（+10 XP/+5 hunger）与 analytics_engine::on_task_completed（递增 daily_stats.tasks_completed + 重算 streak）；或在 ipc::commands::update_task 命令层于 update_task 成功后调用这两个函数（best-effort，宠物未初始化时忽略 Err）。当前这两个函数已定义（pet_engine.rs:151、analytics_engine.rs:86）但从未被调用。
  - [x] SubTask 29.2: 接入 analytics_engine::on_focus_completed。focus_engine::complete_focus_session 已调用 pet_engine::on_focus_completed 并发布 FocusCompleted 事件，但 analytics_engine::on_focus_completed（analytics_engine.rs:105，累加 daily_stats.total_focus_time）未接入。修复方式：在 complete_focus_session 中 `let _ = analytics_engine::on_focus_completed(conn, actual_duration);`；或在 scheduler 中订阅 EventBus::FocusCompleted 事件调用之。
  - [x] SubTask 29.3: 修复 commands.rs 中 data_port 模块解析错误（E0433 ×4）。commands.rs 第 964/972/977/980 行使用裸 `data_port::` 应改为 `core::data_port::`（data_port.rs 位于 src/core/data_port.rs）。
  - [x] SubTask 29.4: 修复 task_engine.rs get_all_tasks 的 stmt 生命周期错误（E0597 ×4，line 96/106）。将 `stmt.query_map(...)?.filter_map(...).collect()` 表达式先赋值给局部变量（如 `let x = ...; x`）再返回，避免块尾临时变量借用问题。
  - [x] SubTask 29.5: 修复 core/data_port.rs:123/137 类型推断错误（E0282 ×2），补充显式类型标注。
  - [x] SubTask 29.6: 核查 tauri_plugin_global_shortcut::init API（lib.rs:20 E0425），可能为插件版本不匹配，按已安装版本调整 API 调用。
  - [x] SubTask 29.7: 修复后重新运行 `cargo check --lib` 确认仅剩预存/环境项错误（core::stats_engine/capture/distill/report/embedding 裸模块路径 E0433 ×8 + 下游 E0282 ×2 + generate_context! proc macro panic ×1 因 ../dist 不存在），共 11 个，无新错误。Vitest 22/22 通过。
